...
SIM_TIME_HORIZON = 50  # Adjust this to change simulation duration -> 50* dt=0.02 = 1 second
    ...
    def compute_cost(reduced_disp):
        ...
        with torch.no_grad():
            ...
            pred, _ = deft_sim.iterative_predict(
                ...
                previous_b_DLOs_vertices_traj=previous_b_DLOs_vertices_traj,
                clamped_positions=clamped_full, dt=0.02,
                parent_theta_clamp=parent_theta_clamp,
                child1_theta_clamp=child1_theta_clamp,
                child2_theta_clamp=child2_theta_clamp,
                inference_1_batch=False
    ...
    def objective(self, x):
        ...
        with torch.no_grad():
            predicted_vertices, _ = self.deft_sim.iterative_predict(
                ...
                inference_1_batch=True # use numba acceleration
            )
...
def trajectory_optimization_ipopt(deft_sim, b_DLOs_vertices_traj, previous_b_DLOs_vertices_traj,
    ...
    with torch.no_grad():
        optimized_traj, _ = problem_obj.deft_sim.iterative_predict(
            ...
            child1_theta_clamp=child1_theta_clamp,
            child2_theta_clamp=child2_theta_clamp,
            inference_1_batch=True # change to numba acceleration
        )
...
def move_bdlo_with_data(checkpoint_path=None):
    ...
    # NOTE: if we run simulation with dt=0.02 but the dataset was recorded at data_dt=0.01,
    # we must downsample frames by frame_stride=2 to match physical time.
    dt = 0.02
    data_dt = 0.01
    total_time = 500
...
def animate_prediction(predicted_vertices, target_vertices, skip_frames=2, title_prefix="Prediction"):
...
if __name__ == "__main__":
    ...
    # Keep dataset sampling consistent with dt used in simulation
    dt = 0.02
    data_dt = 0.01
    frame_stride = int(round(dt / data_dt))
    total_time = 500
    ...
    print(f"Success count: {success_count} out of {number_test} tests.")